    ...
    public class ColaboradorRepository : BaseRepository<Colaborador>, IColaboradorRepository
    {
        ...
        public override async Task<Colaborador> Adicionar(Colaborador entity)
        {
            try
            {
                ...
                ? $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, foto, admissao, tipo, vinculo) "
                + "OUTPUT INSERTED.id_colaborador "
                + "VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Foto, @Admissao, @Tipo, @Vinculo);"
                : $"INSERT INTO {TableName} (cpf, telefone, nome, nascimento, email, logradouro_id, numero, complemento, foto, admissao, tipo, vinculo) "
                + "VALUES (@Cpf, @Telefone, @Nome, @Nascimento, @Email, @LogradouroId, @Numero, @Complemento, @Foto, @Admissao, @Tipo, @Vinculo); "
        ...
        protected override async Task<Colaborador> MapAsync(DbDataReader reader)
        {
            try
            {
                ...
                tipo: (ETipoColaboradorEnum)Convert.ToInt32(reader["tipo"]),
                vinculo: (ETipoVinculoEnum)Convert.ToInt32(reader["vinculo"])
                );
                // Define o ID usando reflection
                var idProperty = typeof(Entity).GetProperty("Id");
